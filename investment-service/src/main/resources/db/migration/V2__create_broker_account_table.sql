...
ADD COLUMN broker_account_id BIGINT NOT NULL REFERENCES broker_account(id) ON DELETE CASCADE;

-- Add comments for clarity
COMMENT ON TABLE broker_account IS 'Stores information about brokerage accounts or bank accounts holding investments.';
COMMENT ON COLUMN broker_account.name IS 'The name for the account.';
...
COMMENT ON COLUMN portfolio.broker_account_id IS 'Link to the broker account holding this portfolio.';