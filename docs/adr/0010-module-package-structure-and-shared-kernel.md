...
- **Hexagonal layout inside each context module:** `domain/model` (entities, value objects), `domain/...` (services, events as they appear), `application/port`, `infrastructure/adapter/...`.
    - **Amendment (2026-08-23), application layer:** when the first application service landed (`RecordBuyService` in `portfolio`), the thin `application/port` was refined for the layer as it materialised: `application/port/in` holds driving ports and their command records, `application/port/out` holds driven ports (e.g. repositories), and `application/service` holds use-case implementations — one class per use case, grouped by cohesion, no `Impl` suffix, each command placed beside its use case in `port/in`. The original single `application/port` predated any application-layer code; the `in`/`out` split and the `service` package are that decision filled in on first contact, not a reversal.
