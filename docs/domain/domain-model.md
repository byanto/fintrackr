...
| `Quantity(BigDecimal value)`                                                     | Shares / units. Lives in the shared kernel: Brokerage's fee API (`computeBuyFee(quantity, price)`) is its second consumer (ADR-010 promotion rule) | value > 0 for transactions; ≥ 0 for derived state |
...
| `reconstitute(id, brokerAccountId, name, tradingBalance, transactions, acquisitions)` | **Not a business method — the persistence load door.** Rebuilds an existing Portfolio from storage: original ids preserved, stored `tradingBalance` restored verbatim (never recomputed from the ledger — ADR-004 amendment), ledger and acquisitions copied defensively. Validates only structural preconditions (non-null required fields); write-time invariants are deliberately skipped so rows written under earlier rules stay loadable. Callable only from the persistence adapter (ADR-009 amendment). |
...
| Method | Purpose |
|---|---|
| `rename(newName)` | Renames the account. Same name rules as at creation (non-null, non-blank), enforced by one shared validation. |
...
| `computeBuyFee(quantity, price)` / `computeSellFee(quantity, price)` | Published fee API for Portfolio Management's use-case services (§5.4). Delegates to `FeeStructure`, which composes `quantity × price × rate` as one raw expression and normalizes to `Money` once (ADR-007 — no intermediate rounding, HALF_EVEN at the boundary). Zero quantity/price yields a zero fee — pure calculation; business rejection stays in the `Portfolio` aggregate. |
